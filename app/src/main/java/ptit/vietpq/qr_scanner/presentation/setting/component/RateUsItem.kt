...
@Preview
@Composable
private fun RateUsItemPreview() {
  RateUsItem()
}

@Composable
fun RateUsItem(modifier: Modifier = Modifier) {
  Column(modifier = modifier) {
    Row(
      horizontalArrangement = Arrangement.SpaceBetween,
      verticalAlignment = Alignment.CenterVertically,
    ) {
      Image(painter = painterResource(id = R.drawable.ic_setting_rate), contentDescription = null)
      Text(
        modifier = Modifier
          .padding(horizontal = 16.dp)
          .weight(1f),
        text = QrLocale.strings.rateUs,
