...
@Composable
fun ContactsContentView(contacts: ContactInfoContent, modifier: Modifier = Modifier) {
  Column(
    ...
    Column {
      if (!contacts.name.isNullOrEmpty()) {
        TwoColorText(QrLocale.strings.name, contacts.name)
      }
      if (!contacts.phone.isNullOrEmpty()) {
        TwoColorText(QrLocale.strings.tel, contacts.phone)
      }
      if (!contacts.email.isNullOrEmpty()) {
        TwoColorText(QrLocale.strings.eMail, contacts.email)
      }
      if (!contacts.address.isNullOrEmpty()) {
        TwoColorText(QrLocale.strings.address, contacts.address)
      }
      if (!contacts.url.isNullOrEmpty()) {
        TwoColorText(QrLocale.strings.url, contacts.url)
      }
    }
  }
}
