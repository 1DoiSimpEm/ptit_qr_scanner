...
@Composable
fun OnLifecycleEventWithBuilder(vararg keys: Any?, builder: LifecycleEventBuilder.() -> Unit) {
  val lifecycleOwner = LocalLifecycleOwner.current
  val lifecycleEventBuilder = remember { LifecycleEventBuilder() }

  // When builder or lifecycleOwner changes or keys change, we need to re-execute the effect
  DisposableEffect(builder, lifecycleOwner, *keys) {
    builder(lifecycleEventBuilder)

    val observer = lifecycleEventBuilder.buildLifecycleEventObserver()
    lifecycleOwner.lifecycle.addObserver(observer)
